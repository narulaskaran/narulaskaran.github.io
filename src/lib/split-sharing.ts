...
export const VALIDATION_LIMITS = {
  ...
  MAX_PEOPLE_COUNT: 50,
  MAX_AMOUNT: 99999.99,
  AMOUNT_TOLERANCE: 0.02, // 2 cent tolerance for rounding (handles tax/tip distribution)
} as const;
...
export function validateSplitDataDetailed(splitData: SharedSplitData): SplitValidationResult {
  ...
  try {
    ...
    if (isNaN(splitData.total)) {
      ...
      // Use more robust floating-point comparison to handle JavaScript precision issues
      // Round both values to 2 decimal places before comparing to avoid floating-point errors
