...
- Nail mode toggle: A nail icon beside the bug control swaps the homepage for the hammer-and-nails layout in `src/components/HammerMode.tsx`. The default card homepage stays the default view; nail mode is opt-in. Hovering a nail shows the project on the hammer badge and a popup; clicking swings the hammer into that nail, then navigates to the project URL. The poster is a fixed light-gray Readymag-style composition, so the light/dark toggle is hidden while it is on. Bug mode and nail mode are mutually exclusive. The choice is stored in `localStorage` under `nailMode`.
